...
P.cache_dir     = './cache/'
P.model_dir     = './models/'
P.model_name    = 'model_2_0d00203746390434.cPickle'
# P.model_name    = None
P.gpu           = 1
P.num_val       = 16
P.datasize      = 128
...
def import_data():
    ...
    #exit()
    if os.path.exists(os.path.join(P.cache_dir, 'x_train_2.npy')):
        x_train = np.load(os.path.join(P.cache_dir, 'x_train_2.npy'))
        y_train = np.load(os.path.join(P.cache_dir, 'y_train_2.npy'))
        s_train = np.load(os.path.join(P.cache_dir, 's_train_2.npy'))
        x_val   = np.load(os.path.join(P.cache_dir, 'x_val_2.npy'))
        y_val   = np.load(os.path.join(P.cache_dir, 'y_val_2.npy'))
        s_val   = np.load(os.path.join(P.cache_dir, 's_val_2.npy'))
    ...
    np.save(os.path.join(P.cache_dir, 'x_train_2'), x_train)
    np.save(os.path.join(P.cache_dir, 'y_train_2'), y_train)
    np.save(os.path.join(P.cache_dir, 's_train_2'), s_train)
    np.save(os.path.join(P.cache_dir, 'x_val_2'), x_val.astype(np.float32))
    np.save(os.path.join(P.cache_dir, 'y_val_2'), y_val.astype(np.float32))
    np.save(os.path.join(P.cache_dir, 's_val_2'), s_val)
        ...
        for i in range(P.datasize/P.batchsize):
            ...
            if iter_num % P.test_interval == 0:
                ...
                if sum_loss_val < min_loss_val:
                    ...
                    fname = 'model_2_' + str(min_loss_val).replace('.', 'd') + '.cPickle'
